	...
	return (
		<div className='space-y-2 w-full'>
			{menuItems?.map((item) => (
				<div
					key={item.id}
					className={`p-2 sm:p-3 cursor-pointer transition-all rounded-xl btnshadow text-[#716F6F] text-[14px] hover:!text-white btnhovershadow duration-200 w-full ${
						item.isActive
				...
				>
					<div className='flex items-center space-x-2 sm:space-x-3 w-full'>
						...
						>
							<div className='w-3 h-3 sm:w-4 sm:h-4'>{item.icon}</div>
						</div>
						<span
							className='font-medium text-xs sm:text-sm leading-relaxed flex-1 truncate'
