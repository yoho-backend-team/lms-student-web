...
const Updates: React.FC = () => {
	...
	const unreadMsgCount =
		activeTab === 'today'
			? data.filter((item: any) => item.status !== 'read')
			: prev.filter((item: any) => item.status !== 'read');
		...
		<div className='flex flex-col gap-5 divshadow w-full h-full rounded-[16px] p-5 overflow-hidden'>
			<div className='flex flex-row justify-between'>
				...
				<p style={{ ...FONTS.para_01 }}>
					<span>{unreadMsgCount.length}</span> New Messages
				</p>
			</div>
			<div className='flex flex-row gap-10'>
				...
				<button
					...
					className={
						activeTab === 'upcoming'
							? `btnshadow text-white btnfocusshadow w-[86px] h-[42px] rounded-xl`
							: `text-[#716F6F] btnshadow w-[86px] h-[42px] rounded-xl btnhovershadow hover:text-white focus:text-white`
					}
				>
					Previous
				</button>
