...
const CourseTrack: React.FC = () => {
	...
	const courseTrack: any = useSelector(
		(state: RootState) => state.CourseSlice.selectedCourse
	);
	...
	const getVideoUrl = courseTrack?.course_modules?.find(
	...
	const formatVideoUrl = (url: string) => {
		...
		return url; // fallback for direct embeddable links
	};
		...
		<div className='w-full mx-auto p-4'>
			...
			<div className='grid grid-cols-1 md:grid-cols-2 gap-8 items-center'>
				<div className='w-full flex justify-center'>
					...
					>
						<iframe
							...
							src={formatVideoUrl(getVideoUrl?.module?.video)}
							title={getVideoUrl?.module?.title}
							frameBorder='0'
				...
				<div className='flex justify-center shadow-[-4px_-4px_4px_rgba(255,255,255,0.7),_5px_5px_4px_rgba(189,194,199,0.75)] rounded-md w-full h-full'>
					<div
						className='relative flex flex-col items-center overflow-auto h-[700px] w-full'
					...
					>
						{courseTrack?.course_modules?.map((item: any, index: number) => {
							...
							return (
								<div
									key={index}
									className='relative z-10 -ml-15 flex items-center my-14 w-full max-w-4xl '
								>
										...
										{isEven ? (
											<div className='mr-10'>
												...
												<img
													...
													className='w-28 h-28 absolute top-8 ml-5 rounded-xl object-contain'
												/>
											</div>
										) : (
									...
									<div className='relative w-0'>
										<div
											className={`w-10 h-10 rounded-full border-4 border-white absolute left-1/2 transform -translate-x-1/2 z-20 ${
												status === 'completed'
										...
										{isEven ? (
											...
											<div className='ml-10'>
												...
												<img
													src={icons}
													alt={item?.module?.title}
													className='w-28 h-28 absolute top-9 ml-12 rounded-xl object-contain'
												/>
											</div>
										)}
									</div>
