...
const SpokenEnglishLearning = () => {
	const engScore: any = useSelector(
		(state: RootState) => state.ProfileSlice.engscore
	);
	const [currentTopic, setCurrentTopic] = useState('Professional Introduction');
	const [currentLevel, setCurrentLevel] = useState('Beginner');
	const [currentMode, setCurrentMode] = useState<'grammar' | 'speaking'>(
		'grammar'
	);
	const [grammarCompleted, setGrammarCompleted] = useState(
		engScore?.grammarCompleted
	);
	const [grammarTestScore, setGrammarTestScore] = useState(
		engScore?.grammarTestScore
	);
	const [unlockedLevels, setUnlockedLevels] = useState<string[]>(
		engScore?.unlockedLevels
	);
	...
	const [achievements, setAchievements] = useState<string[]>(
		engScore?.achievements
	);
	const [lastPracticeDate, setLastPracticeDate] = useState<Date | null>(
		engScore?.lastPracticeDate
	);
	...
	useEffect(() => {
		(async () => {
			...
			setLastPracticeDate(
				engScore?.lastPracticeDate ? new Date(engScore?.lastPracticeDate) : null
			);
			if (
				!engScore?.unlockedTopics[engScore?.unlockedLevels[0]]?.includes(
					currentTopic
				)
			) {
				const firstTopic =
					engScore?.unlockedTopics[engScore?.unlockedLevels[0]][0];
			...
			(async () => {
				...
				if (lastPracticeDate) {
					const daysDiff = Math.floor(
						(today.getTime() - lastPracticeDate.getTime()) /
							(1000 * 60 * 60 * 24)
					);
					if (daysDiff === 1) {
	...
	const getAchievementInfo = (id: string) => {
		...
		> = {
			...
			'grammar-guru': {
				title: 'Grammar Guru',
				description: 'Completed grammar test',
	...
	const onSpeakingUpdate = async (updates: {
		...
		if (updates.levelScores)
			setLevelScores((prev) => ({ ...prev, ...updates.levelScores }));
			...
			<div className='flex flex-col lg:flex-row gap-6 h-[calc(100vh-8rem)]'>
				<div className='w-full lg:w-80 flex-shrink-0 lg:h-full'>
					<div className='lg:sticky lg:top-0 lg:h-[calc(100vh-8rem)] lg:overflow-y-auto'>
						...
						>
							<h2 style={{ ...FONTS.heading_03 }} className='mb-4'>
								Learning Path
							</h2>
												...
												<div className='ml-9 mt-2 space-y-2'>
													{topics['Beginner' as Level]?.map((topic: any) => {
														const isTopicUnlocked =
															unlockedTopics?.[currentLevel as Level]?.includes(
																topic
															);
														...
														return (
															<div
																...
																onClick={() =>
																	isTopicUnlocked && setCurrentTopic(topic)
																}
																style={{
						...
						>
							...
							<div className='grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-4'>
								...
								>
									...
									<p
										style={{ ...FONTS.para_03, fontSize: '11px' }}
								...
								<div className='flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto'>
									<Button
										...
										style={{
											...
											transform:
												currentMode === 'grammar' ? 'scale(1.02)' : 'scale(1)',
											...FONTS.para_02,
											color:
												currentMode === 'grammar'
													? COLORS.white
									...
									<Button
										onClick={() =>
											grammarCompleted && setCurrentMode('speaking')
										}
										...
										style={{
											background:
												currentMode === 'speaking'
													...
													: COLORS.bg_Colour,
											border: `2px solid ${
												grammarCompleted ? COLORS.light_green : COLORS.text_desc
											}`,
											boxShadow:
												currentMode === 'speaking'
													...
													: `rgba(255, 255, 255, 0.7) -2px -2px 4px, rgba(189, 194, 199, 0.75) 2px 2px 4px`,
											transform:
												currentMode === 'speaking' ? 'scale(1.02)' : 'scale(1)',
