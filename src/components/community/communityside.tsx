...
const CommunitySide: React.FC<Props> = ({
  ...
  const handleSendMessage = () => {
    if (!socket || !selectedChat || !inputMessage.trim()) return;
            ...
            >
              ...
              <div className='flex flex-col items-end'>
                <p className='text-xs text-gray-500'>
                  {chat.last_message?.timestamp && formatMessageDate(chat.last_message.timestamp)}
                </p>
                <img
                  src={doubleicon}
            ...
            <div className='flex-1  bg-[#EBEFF3]  overflow-y-scroll  scroll-smooth relative'>
              ...
              >
                {messages?.map((message, index) => (
                  <div
                    key={index}
                    className={`p-2 rounded-lg max-w-[40%] shadow ${isMine(message) ? 'ml-auto bg-blue-100' : 'mr-auto bg-white'
                      }`}
                  >
