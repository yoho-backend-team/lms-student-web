	...
	const activityLogs = [
		{
			...
			description: "Dashboard Logout Successfully musk@gmail.com 25 June 2025 9:40 AM"
		},
		{
			id: 2,
			title: "Dashboard Logout Successfully",
		...
		<div className='py-6 flex gap-6'>
			...
			<div className='w-3/4'>
				...
				<div className='mt-10 relative'>
					<div className='btnshadow h-[100%] w-3 rounded-2xl  absolute left-[29%] text-transparent'> l</div>

					{activityLogs.map((data) => (


						<section key={data.id} className='flex justify-between items-start py-6 gap-12 my-8'>
							<div className='w-[240px]'>
								<p style={{ ...FONTS.para_01 }}>{data.date}</p>
							</div>

							<div className='grid gap-6 w-3/4'>
								<h3 className='pl-14' style={{ ...FONTS.para_01, color: COLORS.text_title }}>{data.title}</h3>
								...
								<section className='flex items-center gap-8'>
									<div className='relative'>
										...
										<div className='h-8 w-8 rounded-full flex p-1 absolute left-[-8px] top-[-5px]' style={{
									...
									<div className='p-3 custom-inset-shadow'>
										<p style={{ ...FONTS.heading_05, color: COLORS.green_text }}>{data.description}</p>
									</div>
								</section>
							</div>
						</section>
			...
			<div className='w-1/4'>
				<div className='flex justify-end cursor-pointer'>
					<img src={filter} alt="" className='h-8 text-end' onClick={() => {
				...
				{showFromCalendar && (
					<div className=' mt-6 p-1 rounded-lg absolute' style={{
						boxShadow:
							'rgba(189, 194, 199, 0.7) 2px 5px 4px,rgba(255, 255, 255, 0.4) 3px 2px 2px inset'
					}}>
