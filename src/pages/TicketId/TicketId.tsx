/* eslint-disable @typescript-eslint/no-explicit-any */
import { useEffect, useRef, useState } from "react";
import { useNavigate, useParams, useLocation } from "react-router-dom";
import ticketicon from "../../assets/icons/Tickets/back.png";
import sendicon from "../../assets/icons/Tickets/Frame 5386.png";
import { Button } from "@/components/ui/button";
import { COLORS, FONTS } from "@/constants/uiConstants";
import {
  Card,
  CardContent,
  CardFooter,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import { GetImageUrl, GetLocalStorage } from "@/utils/helper";
import { useStudentSocket } from "@/context/socketContext";
import { toast } from "react-toastify";


const TicketId = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const location = useLocation();
  const [ticket, setTicket] = useState<any>(location.state || {});
  const [message, setMessage] = useState("");
  const endOfMessageRef = useRef<HTMLDivElement | null>(null);
  const socket = useStudentSocket();
  const studentData = GetLocalStorage('userData')

  const handleBack = () => {
    navigate(-1);
  };

  const formatDate = (dateString: string) => {
    const date = new Date(dateString);
    return date.toLocaleDateString("en-US", {
      day: "numeric",
      month: "short",
      year: "numeric",
      hour: "2-digit",
      minute: "2-digit",
    });
  };

  const getSenderName = (senderType: string) => {
    switch (senderType) {
      case "InstituteAdmin":
        return "Admin";
      case "Instituteuserlist":
        return studentData?.full_name || 'Student';
      default:
        return "Unknown";
    }
  };

  useEffect(() => {
    if (!socket) return;

    socket.connect();

    socket.on("connect", () => {
      socket.emit("joinTicket", ticket?.uuid);
    });

    socket.on("receiveStudentTicketMessage", (new_message: any) => {
      setTicket((prev: any) => ({
        ...prev,
        messages: [...(prev?.messages || []), new_message],
      }));
    });

    return () => {
      socket.disconnect();
      socket.off("receiveStudentTicketMessage");
    };
  }, [socket, ticket?.uuid]);

  useEffect(() => {
    if (endOfMessageRef.current) {
      endOfMessageRef.current.scrollIntoView({ behavior: "smooth" });
    }
  }, [ticket?.messages]);

  const handleSendMessage = () => {
    if (!socket) return;
    try {
      if (message.trim()) {
        const new_message = {
          ticket_id: ticket?.uuid,
          text: message,
          senderType: "Instituteuserlist",
          user: studentData?._id,
        };
        console.log(ticket)
        console.log(new_message)
        setTicket((prev: any) => ({
          ...prev,
          messages: [...(prev?.messages || []), { ...new_message, createdAt: new Date() }],
        }));
        socket.emit("sendStudentTicketMessage", new_message);
        setMessage("");
      } else {
        toast.error("Message can't be empty", { position: "top-center" });
      }
    } catch (error: any) {
      toast.error(error?.message || "Failed to send");
    }
  };

  return (
    <div className="p-6">
      {/* Header */}
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          <img
            src={ticketicon}
            alt="Back"
            className="cursor-pointer"
            onClick={handleBack}
          />
          <h1
            className="text-2xl font-semibold"
            style={{ ...FONTS.heading_01 }}
          >
            Ticket
          </h1>
        </div>
        <h2
          className="text-lg font-semibold text-gray-700"
          style={{
            ...FONTS.heading_04,
            color: COLORS.blue_02,
            fontSize: "24px",
          }}
        >
          TICKET ID: Ticket #{id}
        </h2>
      </div>

      {/* Main Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Chat Card */}
        <Card className="bg-[#ebeff3] h-[550px] shadow-[-4px_-4px_4px_rgba(255,255,255,0.7),_5px_5px_4px_rgba(189,194,199,0.75)]">
          <CardHeader>
            <CardTitle>
              <h1>
                #{id}{" "}
                {ticket?.query ||
                  "This ticket was created from the student Mobile App"}
              </h1>
            </CardTitle>
          </CardHeader>

          <CardContent className="overflow-y-auto h-[500px] pb-4 scrollbar-hide">
            <div className="flex flex-col gap-4">
              {ticket?.messages?.map((msg: any, index: number) => (
                <Card
                  key={index}
                  className={`w-[250px] h-auto ${msg.senderType === "InstituteAdmin"
                    ? "self-start ml-10"
                    : "self-end mr-10"
                    } bg-[#ebeff3] shadow-[-4px_-4px_4px_rgba(255,255,255,0.7),_5px_5px_4px_rgba(189,194,199,0.75)]`}
                >
                  <CardContent className="pb-3 ">
                    <div className="flex justify-between items-center mb-2 ">
                      <p
                        className="text-sm !text-gray-800 font-medium"
                        style={{ ...FONTS.para_01, fontSize: "15px" }}
                      >
                        {getSenderName(msg.senderType)}
                      </p>
                      <p
                        className="text-xs !text-gray-500"
                        style={{ ...FONTS.para_01, fontSize: "12px" }}
                      >
                        {formatDate(msg.createdAt || msg.date)}
                      </p>
                    </div>
                    <p
                      className="text-sm !text-gray-700 pt-6 pl-6"
                      style={{ ...FONTS.para_01, fontSize: "14px" }}
                    >
                      {msg.content || msg.text}
                    </p>
                  </CardContent>
                </Card>
              ))}
              <div ref={endOfMessageRef} />
            </div>
          </CardContent>

          {/* Footer Input */}
          <CardFooter className="px-4">
            <div className="flex items-center w-full gap-3 flex-wrap sm:flex-nowrap">
              {/* Message Icon */}
              {/* <div className="p-2 bg-[#ebeff3] rounded-md flex-shrink-0">
                <img
                  src={messageicon}
                  alt="Message"
                  className="w-10 h-10 sm:w-12 sm:h-12 object-contain cursor-pointer"
                />
              </div> */}

              {/* Input Box */}
              <div
                className="flex-1 bg-[#ebeff3] rounded-md shadow-[inset_2px_2px_5px_rgba(189,194,199,0.75),3px_3px_5px_rgba(255,255,255,0.7)] px-3 py-2"
                style={{ ...FONTS.para_01, fontSize: "15px" }}
              >
                <input
                  type="text"
                  placeholder="Say Something..."
                  className="w-full bg-transparent focus:outline-none text-sm !text-gray-800"
                  value={message}
                  onChange={(e) => setMessage(e.target.value)}
                  onKeyDown={(e) =>
                    e.key === "Enter" ? handleSendMessage() : null
                  }
                />
              </div>

              {/* Send Icon */}
              <div
                className="p-2 bg-[#ebeff3] rounded-md flex-shrink-0 cursor-pointer"
                onClick={handleSendMessage}
              >
                <img
                  src={sendicon}
                  alt="Send"
                  className="w-10 h-10 sm:w-12 sm:h-12 object-contain"
                />
              </div>
            </div>
          </CardFooter>
        </Card>

        {/* Details Card */}
        <Card className="bg-[#ebeff3] p-4 rounded-xl w-full shadow-[-4px_-4px_4px_rgba(255,255,255,0.7),_5px_5px_4px_rgba(189,194,199,0.75)]">
          <CardContent className="space-y-4">
            <div>
              <label className="text-sm font-medium !text-gray-800 block mb-1">
                Issue Description
              </label>
              <div className="bg-[#ebeff3] text-sm !text-gray-700 mt-6 rounded-md p-3 shadow-[3px_3px_5px_rgba(255,255,255,0.7),inset_2px_2px_3px_rgba(189,194,199,0.75)]">
                {ticket?.description || "No description available"}
              </div>
            </div>

            <div>
              <label className="text-sm font-medium !text-gray-800 block mb-1">
                Issue Category
              </label>
              <div className="bg-[#ebeff3] text-sm mt-6 !text-gray-700 rounded-md p-3 shadow-[3px_3px_5px_rgba(255,255,255,0.7),inset_2px_2px_3px_rgba(189,194,199,0.75)]">
                {ticket?.category || "General"}
              </div>
            </div>

            <div>
              <label className="text-sm font-medium !text-gray-800 block mb-1">
                Attachments
              </label>
              <div className="bg-[#ebeff3] text-sm !text-gray-700 mt-6 rounded-md p-3 flex justify-between items-center shadow-[3px_3px_5px_rgba(255,255,255,0.7),inset_2px_2px_3px_rgba(189,194,199,0.75)]">
                <span>{ticket?.file ? "Attachment available" : "No Attachment"}</span>
                {ticket?.file && (
                  <a
                    href={GetImageUrl(ticket.file) ?? undefined}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="!text-[#7b00ff] text-sm font-medium hover:underline"
                  >
                    View
                  </a>
                )}
              </div>
            </div>

            <div>
              <label className="text-sm font-medium !text-gray-800 block mb-1">
                Status
              </label>
              <div className="bg-[#ebeff3] rounded-md p-2 mt-5 w-full shadow-[3px_3px_5px_rgba(255,255,255,0.7),inset_2px_2px_3px_rgba(189,194,199,0.75)]">
                <Button className="px-2 py-1 text-xs rounded-sm bg-gradient-to-l from-[#7B00FF] to-[#B200FF] !text-white 
                  shadow-[0px_2px_4px_0px_rgba(255,255,255,0.75)_inset,3px_3px_3px_0px_rgba(255,255,255,0.25)_inset,-8px_-8px_12px_0px_#7B00FF_inset,-4px_-8px_10px_0px_#B200FF_inset,4px_4px_8px_0px_rgba(189,194,199,0.75),8px_8px_12px_0px_rgba(189,194,199,0.25),-4px_-4px_12px_0px_rgba(255,255,255,0.75),-8px_-8px_12px_1px_rgba(255,255,255,0.25)] 
                  hover:opacity-90 transition">
                  {ticket?.status
                    ? ticket.status.charAt(0).toUpperCase() +
                    ticket.status.slice(1)
                    : "Unknown"}
                </Button>
              </div>
            </div>

            <div>
              <label className="text-sm font-medium !text-gray-800 block mb-1">
                Messages
              </label>
              <div className="bg-[#ebeff3] text-sm !text-gray-700 rounded-md mt-5 p-3 shadow-[3px_3px_5px_rgba(255,255,255,0.7),inset_2px_2px_3px_rgba(189,194,199,0.75)]">
                {ticket?.messages?.length || 0}
              </div>
            </div>
          </CardContent>
        </Card>
      </div>
    </div>
  );
};

export default TicketId;
